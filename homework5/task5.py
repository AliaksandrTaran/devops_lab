...
def date_pars(some_list):  # Function that processing a string and convert it to datetime.datetime format for futher processing
...
def closed_req(close_req): # Function that get valueus from datetime.datetime objects
...
if __name__ == "__main__":
    try:
        name = raw_input('Input GitHub name:')
        passwd = raw_input('Input GitHub password:')
        i = 1
        re = requests.get('https://api.github.com/repos/alenaPy/devops_lab/pulls?page=1&per_page=7&state=all',auth=(name, passwd))
        req = re.json()
        while re.content != '[]': # Request pages while they are not empty
    ...
    system('clear')
    user = raw_input('Enter GitHub user to get statistic about him: ')
    ...
    for i in range(int(num_of_pull_req)):
        login.append(req[i]['user']['login'])  # List contains all users thar makes pull request
        ...
        for k in range(len(date_parsed)):
            hours_create.append(date_parsed[k].hour)  # List contains hour of creating pull request
            week_od_day_opened.append(date_parsed[k].isoweekday())  # List contains day of the week when pull request were created
            week_day.append(date_parsed[k].isocalendar()[1])  # List contains in which week you make pull request
            number_days_created.append(((datetime.datetime.now()) - date_parsed[k]).days)  # List contains number of days from creating the pull request
            merged.append(merged_parswd[k])  # list that contains status about merged/closed

    closed_week, closed_day = closed_req(closed_time)
    if user in login:
        for k in range(len(login)):
            if login[k] == user:
                ids.append(k)  # List contain all users that made requests
    else:
        print('No such user')
        exit(0)

    while (True):  # Menu for user
